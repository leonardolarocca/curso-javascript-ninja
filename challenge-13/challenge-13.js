...
(() => {
  ...
  */
  ...
  let sudeste = [`São Paulo`, `Minas Gerais`, `Rio de Janeiro`, `Espirito Santo`]
  ...
  */
  ...
  let result = brasil.every(estado => {
  ...
  */
  ...
  result = brasil.some(estado => {
    return estado === `Ceará`;
  });
  result ? console.log(`Ceará está incluído!`) : console.log(`Ceará não foi incluído :(`);
  ...
  */
  let map = newBrasil.map((obj, id) => {
    return {id: id+1, estado: `${obj.estado} pertence ao brasil`}
  });
  ...
  */
  let filter = map.filter(obj => {
    return obj.id % 2 ? '' : obj.estado;
  });
